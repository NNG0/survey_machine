...
class PaperManager:
    ...
    def delete_paper(self, paper_id: str) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                try:
                    row = conn.execute(
                        "SELECT file_path FROM papers WHERE id = ?", (int(paper_id),)
                    ).fetchone()
                except ValueError:
                    row = conn.execute(
                        "SELECT file_path FROM papers WHERE url = ?", (paper_id,)
                    ).fetchone()
                ...
                try:
                    cursor = conn.execute(
                        "DELETE FROM papers WHERE id = ?", (int(paper_id),)
                    )
                except ValueError:
                    cursor = conn.execute(
                        "DELETE FROM papers WHERE url = ?", (paper_id,)
                    )
    ...
    def delete_all_papers(self) -> int:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                rows = conn.execute(
                    "SELECT file_path FROM papers WHERE file_path IS NOT NULL"
                ).fetchall()
    ...
    def upsert_workflow_result(
        ...
        research_question = request_status_dict.get("settings", {}).get(
